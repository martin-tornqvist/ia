...
void MapBuild::buildForestTrees(const coord& stairsDown) {
	unsigned minPathLength = 1;//30;
	unsigned maxPathLength = 999;//60;
	...
	while(proceed == false) {
		...
		eng->featureFactory->spawnFeatureAt(feature_stairsDown, stairsDown);

		buildFromTemplate(stairsDown + coord(-14, -12), templateDevName_church, false);

		bool blockers[MAP_X_CELLS][MAP_Y_CELLS];
		eng->mapTests->makeMoveBlockerArrayForMoveType(moveType_walk, blockers);

		path = eng->pathfinder->findPath(eng->player->pos, blockers, stairsDown);
...
void MapBuild::buildForest() {
	...
	coord stairCell(MAP_X_CELLS - 18, 14);
