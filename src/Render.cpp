                            ...
                            (featureId == FeatureId::wall || isHiddenDoor))
                        {
                            if (Map::cells[x][y + 1].isExplored)
                            {
                                const bool IS_SEEN_BELOW  =
                                    Map::cells[x][y + 1].isSeenByPlayer;

                                const auto tileBelowSeen  =
                            ...
                            else //Cell below is not explored
                            {
                                ...
                                tmpDrw.tile =
                                    static_cast<const Wall*>(wall)->getFrontWallTile();
                            }
                        }
                    }
                }
