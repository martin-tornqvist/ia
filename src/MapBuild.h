...
public:
	...
	void makePathByRandomWalk(int originX, int originY, int len, Feature_t featureToMake, const bool TUNNEL_THROUGH_ANY_FEATURE, const bool ONLY_STRAIGHT = true, const coord x0y0Lim =
			coord(1, 1), const coord x1y1Lim = coord(MAP_X_CELLS - 2, MAP_Y_CELLS - 2));

private:
	void makeStraightPathByPathfinder(const coord origin, const coord target, Feature_t feature, const bool SMOOTH, const bool TUNNEL_THROUGH_ANY_FEATURE);

	void buildForestLimit();
	void buildForestOuterTreeline();
	void buildForestTreePatch();
	void buildForestTrees(const coord& stairsDown);
	...
	void addJunctionsToArea(MapArea& area, coord upperLeft, coord lowerRight, const int width, const int height, const Directions_t areaDirection);
