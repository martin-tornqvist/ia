...
void player_jam_feature(Feature* const feature)
{
    ...
    {
        const bool PLAYER_IS_BLIND = map::player->prop_handler().allow_see();
...
void player_jam()
{
    ...
    if (!map::player->inv().has_item_in_backpack(Item_id::iron_spike))
    {
    ...
    render::draw_map_and_interface();
    const Pos jam_pos(map::player->pos + dir_utils::offset(query::dir()));
    msg_log::clear();
