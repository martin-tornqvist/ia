...
DungeonMaster(Engine* engine) : eng(engine) {init();}
   ...
   int getXp() const {return playerExp;}

   int getLevel() const {return playerLvl;}

   int getXpToNextLvl() const {return expTable[playerLvl];}

   int getXpToNextLvlAtLvl(int lvl) const {return expTable[lvl];}

   int getNrOfTraitsToAllocate() const {return playerNrOfTraitsToAlloc;}
