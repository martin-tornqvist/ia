...
namespace LookBecomePlayerAware {

bool action(Monster& monster) {
...
} //lookBecomePlayerAware

namespace CastRandomSpellIfAware {

bool action(Monster* monsterActing) {
...
} //castRandomSpellIfAware

namespace HandleClosedBlockingDoor {

bool action(Monster& monster, vector<Pos> path) {
...
} //HandleClosedBlockingDoor

namespace HandleInventory {
...
bool checkIfBlockingMon(const Pos& pos, Monster* other,
  ...
  engine.lineCalc->calcNewLine(
    other->pos.player->pos, true, 9999, false, line);
...
namespace MoveToRandomAdjacentCell {
...
namespace MoveTowardsTargetSimple {
...
namespace SetPathToLairIfNoLosToLair {

void learn(Monster& monster, vector<Pos>& path, const Pos& lairCell) {
...
} //SetPathToLairIfNoLosToLair

namespace SetPathToLeaderIfNoLosToleader {

void learn(Monster& monster, vector<Pos>& path) {
...
} //SetPathToLeaderIfNoLosToleader

namespace SetPathToPlayerIfAware {
...
void learn(Monster& monster, vector<Pos>& path) {
  if(monster.deadState == ActorDeadState::alive) {
    if(monster.awareOfPlayerCounter_ > 0) {
      ...
      PathFind::run(monster.pos.player->pos, blockers, path);
    } else {
      path.resize(0);
    }
  } else {
...
} //SetPathToPlayerIfAware

namespace SetSpecialBlockedCells {

void learn(Monster& monster, bool a[MAP_W][MAP_H]) {
  (void)monster;
  (void)a;
//    Utils::resetArray(a, false);
}

} //SetSpecialBlockedCells
...
namespace StepToLairIfHasLosToLair {

bool action(Monster& monster, const Pos& lairCell) {
  if(monster.deadState == ActorDeadState::alive) {
    bool blockers[MAP_W][MAP_H];
    MapParse::parse(CellPred::BlocksVision(), blockers);
